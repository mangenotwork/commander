...
func (idw *IdWorker) InitIdWorker(workerId, datacenterId int64) error {
	...
	if idw.workerId < 0 || idw.workerId > idw.maxWorkerId {
		return errors.New(fmt.Sprintf("workerId[%v] is less than 0 or greater than maxWorkerId[%v].",
			workerId, datacenterId))
	}
	if idw.datacenterId < 0 || idw.datacenterId > idw.maxDatacenterId {
		return errors.New(fmt.Sprintf("datacenterId[%d] is less than 0 or greater than maxDatacenterId[%d].",
			workerId, datacenterId))
	}
...
func (idw *IdWorker) NextId() (int64, error) {
	...
	if timestamp < idw.lastTimestamp {
		return -1, errors.New(fmt.Sprintf("Clock moved backwards.  Refusing to generate id for %d milliseconds",
