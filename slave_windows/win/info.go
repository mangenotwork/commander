...
func GetNetIOFromCMD() (networks []*entity.NetWorkIO) {
	...
	networks = make([]*entity.NetWorkIO, 0)

	networks = append(networks, &entity.NetWorkIO{
		Name: "网络",
		Tx:   outputMS,
		Rx:   inputMS,
	})
...
func GetProcessList(pg int) (pBaseInfo []*entity.ProcessBaseInfo){
	...
	count, pidList := GetWindowsPIDInfo()
	log.Println("GetProcessList count, pidList = ", count, pidList)
	for i, v := range pidList{
		// 先限制到 100个
		...
		data = append(data, &entity.ProcessBaseInfo{
			PID : vList[0],
			PName: vList[1],
		})
	}
